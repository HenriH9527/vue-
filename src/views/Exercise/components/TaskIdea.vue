<template>
  <div class="TaskShare">
    ...
    <div class="btn">
      ...
      <span class="praise" @click="toZan"><svg-icon :icon-class="iconClass"/> {{news.agree_number||'0'}}</span>
...
import { getTaskCmt, publishTaskCmt } from '@/api/task';
  ...
  methods: {
    ...
    async showDiscuss() {
      try {
        if (this.open === false) {
          if (!this.cmts.length) {
            const { data } = await getTaskCmt({ dynamic_id: this.newss.id });
            this.cmts = [...data];
          }
          this.open = true;
        } else {
