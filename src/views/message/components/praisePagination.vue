...
export default {
  ...
  computed: {
    ...
    nums() {
      ...
      if (totalNum <= this.boundaryNum * 2) return totalNum;
      if (currentNum <= this.boundaryNum) {
        ...
        return [1, '…'].concat(new Array(this.boundaryNum * 2 - 1).fill(totalNum).map((item, index) => item - index).reverse());
      } return [1, '…'].concat(new Array(this.boundaryNum).fill(currentNum - 1).map((item, index) => item + index)).concat('…', totalNum);
    },
    // 获取总前总列表数量
    totalTask() {
      return this.$store.state.msg.praiseNum;
    },
    // 计算分页数量
    totalPage() {
      return Math.ceil(this.totalTask / this.showNum);
    },
    // 当msgPraise.vue加载完再显示Pagination
    showPagination() {
      return this.$store.state.msg.showPraisePagination;
    },
  },
  methods: {
    // 点击数字按钮传送当前页码到父组件中获取当前需展示的页面内容
    select(num) {
